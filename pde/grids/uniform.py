...
class uniformgrid(grid,active_manip):
	...
	def __init__(self, dims, type='d',x=None,rx=None ):
		"""Constructor for a uniform grid.
			...
			initialized to zero.  Note that the active set by default limits index values to be 16 
			bit integers, which gives us a large space over which we may calculate the solution, 
			but could run out for enormous grid sizes.
...
class twoddir(uniformgrid):
	...
	def _set_bc_left(self,fn):
		"""Internal: function which sets boundary values according to fn on the left side of the
			boundary (u[:,0])
		"""
#		print numpy.s_[0:]
	...
	def _set_bc_right(self,fn):
		"""Internal: function which sets boundary values according to fn on the right side of the
			boundary (u[:,-1])
		"""
	...
	def set_lat_bc(self,*args):
		"""Set function values on the lateral boundaries (u[0,:] and u[-1,:].)
			Takes a variable number of arguments depending on the preferences of the user.  If one
			argument is given, it should be a function defined everywhere in the domain (or at least
			on the complement of the active indices in the domain) which gives the boundary value 
			there.  If two arguments are given, they should be the left and right boundary values, 
			respectively.  Mixing these is a very bad idea.
			
			As explained in the documentation for set_bc, the complication and chance of making an
			error is much increased when using set_lat_bc, so it should be avoided.
		"""
...
#		self._set_bc_left()
	...
	def set_test_linorder(self):
		"""Sets the grid to a test dataset """
		l=map(tuple,
	...
	def set_test_xprod(self):
		l=map(tuple,
			numpy.transpose(
						numpy.indices(self.u.shape)
						).reshape((numpy.prod(self.nx),2)))
		for i in l:
			self.u[i]=numpy.prod(self.ind_to_x(i))