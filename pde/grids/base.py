...
class grid(trivgrid):
	...
	def _set_active_xv(self, xv):
		...
		self.l.info('Setting active area by a list of x values.')
...
class active_manip(object):
	def set_active_mat(self, m):
		...
		"""
		try:
			self.l.debug('Setting active area to matrix \n {}'.format(m))
		except AttributeError:
			pass
		self._set_active_ind(numpy.transpose(numpy.nonzero(m)))
	
	def set_active_all(self):
		"""Set domain interior (self.active) to be whole domain."""
		try:
	...
	def set_active_int(self):
		"""Set domain interior (self.active) to be all but extreme points."""
		try:
		...
		def domfn(x):
			...
			for i in range(0, self.u.ndim):
#				print x[i], self.rx[i][0]>=x[i] or self.rx[i][1]<=x[i]
	...
	def set_active_int_plus_top(self):
		...
		def domfn(x):
			...
			if x[ax] == self.rx[ax][0]:
				return False
			for i in numpy.setdiff1d(range(0, self.u.ndim), [ax]):
#				print x[i], self.rx[i][0]>=x[i] or self.rx[i][1]<=x[i]
				if x[i] <= self.rx[i][0] or x[i] >= self.rx[i][1]:
					return False
			return True	
	...
	def set_active_eq(self, domfn):
		...
		"""
		try:
			self.l.info('Setting active area according to domfn.')
			self.l.debug('Doc string: {}'.format(domfn.__doc__))
		except AttributeError:
			pass
		td = numpy.array(map(
					self.ind_to_x,
					numpy.transpose(
									numpy.indices(self.nx)
									).reshape((numpy.prod(self.nx), self.u.ndim))))
#		kp=numpy.array(map(domfn,td))
		tmp = []
		for i in td:
			if domfn(i):
				tmp.append(i)
		#print numpy.vstack(tmp[:])
		self._set_active_xv(numpy.vstack(tmp[:]))

	def set_active_draw(self):
			#TODO: Implement "drawn" active sets with ginput
			raise NotImplementedError('set_active_draw not yet implemented')
